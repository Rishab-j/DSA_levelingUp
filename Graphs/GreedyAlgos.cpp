...
{
public:
    bool operator()(const DijkstraPair &self, const DijkstraPair &other) const
    {
...
void DijkstraAlgo(int src, vector<bool> &vis)
{
    vector<vector<Edge>> dijkstraGraph(N, vector<Edge>());
    
    priority_queue<DijkstraPair, vector<DijkstraPair>, compareTo2> que;
    ...
    while (noOfEdges < noOfVertex - 1)
    {
        ...
        {
            if (!vis[e.v])
                que.push(DijkstraPair(e.v, p.u, e.w , p.wsf + e.w));
        }
    }
