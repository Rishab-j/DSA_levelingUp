...
int dfs_JourneyToMoon(vector<vector<int>> &graph, int src, vector<bool> &vis)
{
    vis[src] = true;
    int count = 0;
    for (int e : graph[src])
...
public:
    int findCheapestPrice(int n, vector<vector<int>> &flights, int src, int des, int K)
    {
        ...
        typedef vector<int> pair; //  wsf,src,stop
...
int networkDelayTime(vector<vector<int>> &times, int N, int K)
{
    ...
    while (que.size() != 0)
    {
        ...
        if (minDisSoFar[rvtx[1]] != -1)
            continue;

        minDisSoFar[rvtx[1]] = rvtx[0];

        for (pii e : graph[rvtx[1]])
        {
            if (minDisSoFar[rvtx[1]] != -1)
                que.push({e.second + rvtx[0], e.first});
        }
    }
    int ans = 0;

    for (int i = 1; i < minDisSoFar.size(); i++)
    {

        if (minDisSoFar[i] == -1)
            return -1;
        else
            ans = max(ans, minDisSoFar[i]);
    }
    return ans;
}