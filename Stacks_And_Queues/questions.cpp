...
int trap_02(vector<int> &heights)
{
    ...
    for (int i = 0; i < heights.size(); i++)
    {
        while (st.size() != 0 && heights[st.top()] < heights[i])
        {
            int h = heights[st.top()];
            st.pop();
            if (st.size() == 0) // pop krne ke baad agr stack khali hojaae to kuch ni krna
                break;
            int w = i - st.top() - 1;
...
// Open link for other solutions https://leetcode.com/problems/min-stack/discuss/820450/4-Solution-or-Node-or-One-Stack-or-Two-Stack-or-Easy-to-Understand
