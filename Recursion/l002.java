...
public class l002 {
  ...
  // 2 calls from a node 
  public static int Nqueen_04_subs(int n, int r, int c, int tnq, String ans) {
    if (tnq == 0 || r >= n || c >= n) { // we dont need to check the condition for rows if the box is n*n and no. of
                                        // queens is also n but , if box size and queens ae different then we have to
                                        // put condition in rows too
      if (tnq == 0) {
        System.out.println(ans);
        return 1;
      }
    ...
    if (!colA[c] && !diagA[r + c] && !adiagA[r - c + n - 1]) {
      ...
      count += Nqueen_04_subs(n, r + 1, 0, tnq - 1, ans + "(" + r + "," + c + ") ");

      colA[c] = !colA[c];
      diagA[r + c] = !diagA[r + c];
      adiagA[r - c + n - 1] = !adiagA[r - c + n - 1];
    }
    count += Nqueen_04_subs(n, r, c + 1, tnq, ans);
    return count;
  }
  ...
  public static void nQueen() {
    ...
    System.out.println(Nqueen_04_subs(n, 0, 0, tnq, ""));
  }
